/* Q9
bsearch 함수와 같은 형식으로 호출할 수 있는 다음 함수를 작성하세요. 이때 Q5(115쪽) 처럼
이진 검색 알고리즘을 사용하여 일치하는 요소의 검색에 성고하면 그 위치에서 앞쪽으로 선형 검색을
수행하여 가장 앞쪽의 요소에 대한 포인터를 반환하세요.
...
int main() {
    ...
    p = (int*)bsearchx(&ky,                                    // 검색값의 메모리 주소값
                 x,                                            // 배열 x
                 nx,                                           // 요소의 개수
                 sizeof(int),                                  // 요소의 크기
